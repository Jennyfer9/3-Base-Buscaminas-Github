...
public class ControlJuego {
	...
	private int calculoMinasAdjuntas(int i, int j) {
		...
		for (int vertical = i - 1; vertical <= i + 1; vertical++) {
			for (int horizontal = j - 1; horizontal <= j + 1; horizontal++) {
				if ((vertical >= 0) && (horizontal >= 0) && (vertical < LADO_TABLERO) && (horizontal < LADO_TABLERO)) {
					if (tablero[vertical][horizontal] == MINA) {
						contadorDeMinas++;
					}
				}
	...
	public boolean esFinJuego() {
		boolean terminar;
		int calculo = (LADO_TABLERO * LADO_TABLERO - MINAS_INICIALES);

		if (calculo == puntuacion) {
			terminar = true;
		} else {
			terminar = false;
		}
